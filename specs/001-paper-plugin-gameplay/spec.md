...
プレイヤーは他チームのプレイヤーを攻撃することも可能だが、ダメージを与えるとMurdererフラグが立ち、厳しいペナルティを受ける。チーム内でのPvPは完全に無効化されている。このシステムにより、PvPは可能だが抑止力が働く。

**Why this priority**: PvPバランスを保つ重要な要素だが、ゲームの中核ではない。

**Independent Test**: 他チームのプレイヤーを攻撃してMurdererフラグが立ち、ペナルティが適用されることを確認できる。

**Acceptance Scenarios**:

1. **Given** プレイヤーが同じチームのプレイヤーを攻撃する、**When** 攻撃を実行する、**Then** ダメージは発生しない（チーム内PvP無効）
2. **Given** プレイヤーが他チームのプレイヤーにダメージを与える、**When** ダメージが発生する、**Then** Murdererフラグが立ち、プレイヤー名が赤字表示になる
3. **Given** プレイヤーがMurdererフラグを持っている、**When** フラグが有効である、**Then** 防具を装備できなくなり、他プレイヤーからペナルティなしで攻撃され、死亡時に装備をドロップする
4. **Given** Murdererフラグが立っている、**When** 死亡するか10分経過する、**Then** Murdererフラグが解除され、通常状態に戻る
...
- **FR-039**: システムは同じチーム内のプレイヤー間のPvPを完全に無効化しなければならない
- **FR-040**: プレイヤーが他チームのプレイヤーにダメージを与えた際、Murdererフラグを立てなければならない
...
- **FR-043**: Murdererフラグは死亡または10分経過で自動解除されなければならない
...
- **FR-076**: プレイヤーがMurdererになった際、全プレイヤーにタイトルメッセージで「[プレイヤー名]がMurdererになった！」と通知しなければならない
